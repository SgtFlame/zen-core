...
#ifdef THREADMODEL_WIN32
    typedef Thread_win32 native_thread_type;
#else
#ifdef THREADMODEL_SOLARIS
    typedef Thread_solaris native_thread_type;
#else
#ifdef THREADMODEL_POSIX
    typedef Thread_posix native_thread_type;
#endif // THREADMODEL_POSIX
#endif // THREADMODEL_SOLARIS
#endif // THREADMODEL_WIN32

I_Thread*
ThreadFactory::create(I_Runnable* const _pRunnable)
{
    return new native_thread_type(_pRunnable);
}

//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
void
ThreadFactory::destroy(I_Thread *const _pThread)
{
    delete dynamic_cast<native_thread_type*>(_pThread);
}
