...
#ifdef THREADMODEL_WIN32
    typedef Mutex_win32 native_mutex_type;
#else
#ifdef THREADMODEL_SOLARIS
    typedef Mutex_solaris native_mutex_type;
#else
#ifdef THREADMODEL_POSIX
    typedef Mutex_posix native_mutex_type;
...
MutexFactory::create()
{
    return new native_mutex_type();
}

//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
void
MutexFactory::destroy(I_Mutex *const _pMutex)
{

    delete dynamic_cast<native_mutex_type*>(_pMutex);
}
